/* ============================================================
   AN Psixoloji — Lisenziya Yoxlama Modulu (license-check.js)
   Bütün ödənişli alətlər bu skripti import edir.
   Hazırda localStorage + statik key bazası ilə işləyir.
   Firebase qoşulanda yalnız LICENSE.checkKey() funksiyası
   dəyişdirilməlidir, qalan hər şey aynı qalır.
============================================================ */
(function(){
  var TOOL_ID = document.currentScript.getAttribute('data-tool') || '';

  var LICENSE = {
    storageKey: 'an_license_' + TOOL_ID,

    // ---- Bu hissə Firebase qoşulanda dəyişəcək ----
    ...
    texts: {
      az: { title:"Lisenziya tələb olunur", desc:"Bu alətdən tam istifadə etmək üçün lisenziya açarını daxil edin.", placeholder:"Lisenziya açarını daxil edin", btn:"Aktivləşdir", error:"Açar yanlışdır və ya bu alət üçün deyil.", expired:"Lisenziyanın müddəti bitib.", contact:"Lisenziya almaq üçün əlaqə saxlayın", success:"Aktivləşdirildi!" },
      en: { title:"License Required", desc:"Enter your license key to unlock full access to this tool.", placeholder:"Enter license key", btn:"Activate", error:"Invalid key or not valid for this tool.", expired:"License has expired.", contact:"Contact us to get a license", success:"Activated!" },
      ru: { title:"Требуется лицензия", desc:"Введите лицензионный ключ для полного доступа к инструменту.", placeholder:"Введите лицензионный ключ", btn:"Активировать", error:"Неверный ключ или не подходит для этого инструмента.", expired:"Срок лицензии истёк.", contact:"Свяжитесь с нами для получения лицензии", success:"Активировано!" }
    },

    getLang: function(){
      try { return (window.currentLang || localStorage.getItem('an_lang') || 'az'); } catch(e){ return 'az'; }
    },
    ...
    init: function(contentSelector){
      if (!this.isActivated()) {
        this.lockContent(contentSelector || 'main');
      }
    }
  };
